...
echo "<div id=div-body class=div-body>";
...
foreach ($tasks->tarefa as $task) {
    if(!strcmp(strval($task->id),$task_id)){
        ...
        if (!strcmp($task->status, "Pendente")){
            echo "<div id='status-div' style='display: ".($can_confirm?"default":"none")."; float:right; cursor:pointer;' onmouseover='hoverconfirm()' onmouseout='hoveroutconfirm()' onclick='clickconfirm(".$task->id.")''> ";
